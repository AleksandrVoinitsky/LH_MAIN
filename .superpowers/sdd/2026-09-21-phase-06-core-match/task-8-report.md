...
fix round 1 process cleanup:
- Re-ran Unity EditMode using the required executable `C:\Program Files\Unity\Hub\Editor\6000.3.14f1\Editor\Unity.exe` and required project/results paths.
- Initial `-quit` run exposed a compile error in `CoreMatchRuntimeTests`; added the missing `LH.Main.Unity.Gameplay.Items` using.
- The `-quit` runs returned without refreshing XML after compilation, so reran the same command without `-quit` as instructed.
- Unity XML result at `task-8-editmode.xml`: `result="Passed" total="134" passed="134" failed="0"`.
- Command: `git diff --check`; result: passed with no output after cleanup.

fix round 1 follow-up commit hashes:
- `812ecca` fix: stabilize core match runtime tests
- `dad163e` chore: untrack task 8 report
