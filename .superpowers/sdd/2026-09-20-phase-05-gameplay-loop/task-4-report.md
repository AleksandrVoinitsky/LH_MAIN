...
## Fix Round 1/5: Server-Owned Life State API

### What Changed

- Changed `NetworkPlayerController.LifeState` from a public setter to a private setter so client-side code cannot assign life state directly through the public API.
- Added `NetworkPlayerController.ApplyServerLifeState(PlayerLifeState)` as the explicit server-owned state mutation path and marked it with FishNet's `[Server]` attribute.
- Added EditMode reflection coverage that verifies `LifeState` is not publicly settable and the mutation method is server-gated.
- Updated the terminal movement test setup to avoid direct public `LifeState` assignment.
...
```powershell
& "C:\Program Files\Unity\Hub\Editor\6000.3.14f1\Editor\Unity.exe" -batchmode -projectPath "D:\LH_MAIN-phase-02" -runTests -testPlatform EditMode -testResults ".superpowers/sdd/2026-09-20-phase-05-gameplay-loop/task-4-fix-round-1-red.xml"
```

RED result:

```xml
<test-run id="2" testcasecount="64" result="Failed(Child)" total="64" passed="63" failed="1" inconclusive="0" skipped="0" ...>
```

Relevant failure:

```text
NetworkPlayerControllerTests.LifeStateCannotBeSetDirectlyThroughPublicApi
Expected: False
But was:  True
```
...
```powershell
& "C:\Program Files\Unity\Hub\Editor\6000.3.14f1\Editor\Unity.exe" -batchmode -projectPath "D:\LH_MAIN-phase-02" -runTests -testPlatform EditMode -testResults ".superpowers/sdd/2026-09-20-phase-05-gameplay-loop/task-4-fix-round-1-green.xml"
```

GREEN result:

```xml
<test-run id="2" testcasecount="65" result="Passed" total="65" passed="65" failed="0" inconclusive="0" skipped="0" ...>
```
...
Result: no whitespace errors; Git printed LF-to-CRLF warnings for the touched tracked files.

### Notes

- FishNet's `[Server]` interceptor expects spawned/network runtime state, so the EditMode behavior test arranges terminal life state through reflection instead of executing the server-gated method on an unattached test object.
